...
class SphereConv2D(nn.Module):
    ...
    def forward(self, x):
        if self.grid_shape is None or self.grid_shape != tuple(x.shape[-2:]):
            self.grid_shape = tuple(x.shape[-2:])
            coordinates = gen_grid_coordinates(x.shape[-2], x.shape[-1], self.stride)
            with torch.no_grad():
                self.grid = torch.FloatTensor(coordinates).to(x.device)
                self.grid.requires_grad = True

        with torch.no_grad():
            grid = self.grid.repeat(x.shape[0], 1, 1, 1).to(x.device)

        x = nn.functional.grid_sample(x, grid, mode=self.mode, align_corners=True)
        x = nn.functional.conv2d(x, self.weight, self.bias, stride=3, groups=1)
        return x
