...
class ResNet(nn.Module):
    ...
    def forward(self, x):       # input x = [1,3,16,256,320]
        x = self.conv1(x)       # x=[1,64,16,128,160] 
        x = self.bn1(x)         # x=[1,64,16,128,160] 
        x = self.relu(x)        # x=[1,64,16,128,160] 
        x = self.maxpool(x)     # x=[1,64,8,64,80] 

        x = self.layer1(x)      # x=[1,64,8,64,80]
        x = self.layer2(x)      # x=[1,128,4,32,40]
        x = self.layer3(x)      # x=[1,256,2,16,20]
        x = self.layer4(x)      # x=[1,512,1,8,10]
...
class ResNetLike(nn.Module):
    ...
    def __init__(self, block, layers, sample_size, sample_duration, shortcut_type='B', num_classes=400, last_fc=True, last_pool=True):
        ...
        last_size = math.ceil(sample_size / 32)
        self.avgpool = nn.AvgPool3d((last_duration, last_size, last_size), stride=1)
        self.fc_new = nn.Linear(256 * block.expansion, num_classes)
    ...
    def forward(self, x):       # input x = [1,3,16,256,320]
        ...
        x = self.layer2(x)      # x=[1,128,4,32,40]
        x = self.layer3(x)      # x=[1,256,2,16,20]
        x = self.layer4(x)      # x=[1,256,1,8,10]
...
def resnet18like(**kwargs):
    """Constructs a ResNet-18 model.
    """
    model = ResNetLike(BasicBlock, [2, 2, 2, 2], **kwargs)
    return model
